...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Initialize inputIndex to the beginning of the user's input in the sql string,
  // Initialize illegalOperand variable to 0 and define a localScript string variable.
  int inputIndex = sql.find("=");
  int illegalOperand = 0;
  std::string localScript;

  // if there is not an " OR " or an " or " string within the sql parameter set illegalOperand to -1
  //    otherwise set illegalOperand to the index of the substring
  if (sql.find(" or ", inputIndex + 1) == std::string::npos) {
      if (sql.find(" OR ", inputIndex + 1) == std::string::npos) {
          illegalOperand = -1;
      }
      else {
          illegalOperand = sql.find(" OR ", inputIndex + 1);
      }
  }
  else {
      illegalOperand = sql.find(" or ", inputIndex + 1);
  ...
  // if an '=' char is detected within the user input section of the sql parameter string
  //    and there currently has not been an illegal operator detected
  //    then set illegalOperand variable to the char's index
  if ((sql.find("=", inputIndex + 1) != std::string::npos) && (illegalOperand == -1)) {
      illegalOperand = sql.find("=", inputIndex + 1);
  }

  // if illegalOperand is not -1, then set the localScript variable to the sql parameter string
  //    excluding the illegal OR/or operator and any chars that follow
  if (illegalOperand != -1) {
      localScript = sql.substr(0, illegalOperand);
  }
  else {
      localScript = sql;        //set localScript variable to the parameter sql string if no illegal operators were detected
  }

  char* error_message;
  if(sqlite3_exec(db, localScript.c_str(), callback, &records, &error_message) != SQLITE_OK)
  {
