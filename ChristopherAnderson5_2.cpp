// ChristopherAnderson5_2.cpp : This file contains the 'main' function. Program execution begins and ends there.
// Encryption cpp file code : This file contains the 'main' function. Program execution begins and ends there.
//
...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    { 
        ...
        output[i] = source[i] ^ key[i % key.size()];
    }
    //std::cout << "Encryption performed" << std::endl;
...
std::string read_file(const std::string& filename)
{
    ...
    std::stringstream fileBuffStream;

    if (inFile.is_open()) {
        file_text = "";

        fileBuffStream << inFile.rdbuf();
        file_text = fileBuffStream.str();

        //std::cout << filename << " read into program" << std::endl;
    }
    else {
        std::cout << "Error accessing " << filename << std::endl;
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    std::string rawTimeStr = ctime( rawTimeDat);
    */
    time_t rawTime = time(NULL);

    char timeStr[26];

    ctime_s(timeStr, sizeof timeStr, &rawTime);

    std::string yearMonthDay = timeStr;
    int spacePos1 = yearMonthDay.find(" ");
    yearMonthDay = yearMonthDay.substr(spacePos1+1);

    int spacePos3 = yearMonthDay.find_last_of(" ");
    spacePos1 = yearMonthDay.find(":");

    std::string yearStr = yearMonthDay.substr(spacePos3 + 1);
    yearMonthDay = yearMonthDay.substr(0, spacePos1 -3);

    std::replace(yearMonthDay.begin(), yearMonthDay.end(), ' ', '-');
    yearStr.append(yearMonthDay);
    yearMonthDay = yearStr;

    std::replace(yearMonthDay.begin(), yearMonthDay.end(), '\n', '-');

    yearMonthDay.erase(yearMonthDay.end() - 3);
    
    yearMonthDay.append("\n");

    //yearMonthDay = yearStr + yearMonthDay;

    std::ofstream outFile;
    outFile.open(filename);

    outFile << student_name << "\n";
    outFile << yearMonthDay << "\n";
    outFile << key << "\n";
    outFile << data << "\n";
